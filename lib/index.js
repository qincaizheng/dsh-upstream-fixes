...
export async function startSideChat(ctx, sessionId, text) {
  ...
  try {
    const { childId } = await subagents.startContinuable({
      ...
      request: {
        parent: agent,
        prompt: textPrompt(contextPrompt(agent, question)),
...
/**
 * Find one child session's persistence directory: session storage lays out
 * as <DSH_HOME>/sessions/<workspace-dir>/<session-id>/, where the workspace
 * dir is an encoded cwd — scanning every workspace dir for the child id
 * sidesteps the encoding entirely.
 */
export function childSessionDir(home, childId) {
...
/**
 * Ephemeral /btw cleanup: after a short read window (the panel shows the
 * answer while it streams, and keeps it visible for a beat after settle),
 * delete the child's persisted session and archive it — no history remains.
 */
export function cleanupEphemeralChild(ctx, childId, delayMs = 30_000) {
  const run = () => {
    try {
      const dir = childSessionDir(dshHome(), childId)
      if (dir !== null) {
        rmSync(dir, { recursive: true, force: true })
        console.log('[dsh-upstream-fixes] removed ephemeral /btw child ' + childId)
      }
      const workspaces = ctx.get?.('workspaces')
      if (workspaces !== undefined && typeof workspaces.archiveSession === 'function') {
        void workspaces.archiveSession(childId).catch(() => {})
      }
    } catch (error) {
      console.warn('[dsh-upstream-fixes] ephemeral /btw cleanup failed: ' + (error instanceof Error ? error.message : String(error)))
    }
  }
 ...
 */
export function sideCommandsDefinition(subagents, hooks = {}) {
  ...
  return [
      ...
      handler: async ({ agent, rawInput, signal }) => {
        ...
        try {
          const { childId } = await subagents.startContinuable({
            provider: FORK_PROVIDER,
            label: shortLabel(question),
            request: { parent: agent, prompt: textPrompt(contextPrompt(agent, question)), signal },
          })
    ...
    {
      name: 'btw',
      description: 'Ask a one-shot question in a background child',
      recordInput: false,
      input: { hint: '<question>' },
      handler: async ({ agent, rawInput, signal }) => {
        const question = rawInput.trim()
        // 不带参数 = 打开侧栏（客户端卡片识别该成功文案并自动弹出面板）。
        if (question === '') return { kind: 'success', text: '已打开侧栏面板' }
        const refuse = missing('后台问答')
        if (refuse !== undefined) return refuse
        try {
          const run = await subagents.start(FORK_PROVIDER, {
            label: shortLabel(question),
            prompt: textPrompt(contextPrompt(agent, question)),
            parent: agent,
            signal,
          })
          // 一次性子会话：结果落盘后即可释放 run；随后由 hook 清理
          // （/btw 不留历史：面板移除 + 归档 + 物理删除持久化记录）。
          const cleanupChild = typeof hooks.cleanupChild === 'function' ? hooks.cleanupChild : () => {}
          void run.result
            .catch(() => {})
            .then(() => run.dispose().catch(() => {}))
            .then(() => { cleanupChild(run.id) })
          return { kind: 'success', text: '已启动后台问答 ' + run.id + '（答案见侧栏面板）' }
        } catch (error) {
          return { kind: 'error', text: '启动后台问答失败：' + (error instanceof Error ? error.message : String(error)) }
        }
      },
    },
    {
...
export function apply(ctx) {
  ...
  ctx.inject(['commands'], (commandCtx) => {
    ...
    const hooks = { cleanupChild: (childId) => cleanupEphemeralChild(commandCtx, childId) }
    for (const definition of sideCommandsDefinition(subagents, hooks)) {
