...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* ---- own side-chat implementation (replaces dsh-sidechain) -----
		   ...
		   The host half registers /side (durable continuable child) on the
		   official subagent fork backend and a GLOBAL dispatch rule on the
		   fork provider; this client half renders the rest, written from
		   scratch:
		...
		*/
		...
		function SideChatPanel(props) {
			...
			return react.createElement("div", { style: panelStyles.root },
				...
				rows.length === 0 ? react.createElement("p", { style: panelStyles.empty }, "暂无侧边会话——在输入框直接对话，或用 /side 启动。") : null,
		...
		/** Command card for /side: outcome text plus a sidebar jump. When the
		    card first mounts with a fresh child id it auto-opens the tab and
		    preselects that child (once per child, per browser tab). */
		...
		/** 设置页「子代理模型」分区：全局子代理派发规则配置（名称 + 描述 + 模型）。
		    全局存储：模型目录来自宿主 /model-catalog（无会话上下文），
		    配置读写走 /subagent-models。 */
		function SideChatSettingsSection() {
			...
			const reload = async () => {
				try {
					const [configRes, catalogRes] = await Promise.all([
						window.fetch("/api/upstream-fixes/subagent-models", { headers: { accept: "application/json" } }),
						window.fetch("/api/upstream-fixes/model-catalog", { headers: { accept: "application/json" } }),
					]);
					const configBody = await configRes.json();
					const catalogBody = await catalogRes.json();
					if (configBody?.ok === true && configBody.models !== null && typeof configBody.models === "object") setConfig(configBody.models);
					if (catalogBody?.ok === true && Array.isArray(catalogBody.groups)) setGroups(catalogBody.groups);
					else if (typeof catalogBody?.message === "string") setError(catalogBody.message);
				} catch { /* 读取失败不阻塞设置页 */ }
			};
			...
			const add = async () => {
				...
				try {
					const response = await window.fetch("/api/upstream-fixes/subagent-models", {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({ name: trimmed, description: desc.trim(), provider, model }),
					});
					const body = await response.json();
					if (body?.ok !== true) throw new Error(body?.message ?? "保存失败");
					setConfig(body.models ?? {});
					setName("");
					setDesc("");
					setPick("");
				} catch (caught) {
			...
			const remove = async (entryName) => {
				setBusy(true);
				try {
					const response = await window.fetch("/api/upstream-fixes/subagent-models", {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({ name: entryName, remove: true }),
					});
					const body = await response.json();
			...
			return react.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 12, maxWidth: 720, color: "var(--dsw-alias-label-primary)" } },
				react.createElement("h2", { style: { margin: 0, fontSize: 18, fontWeight: 600 } }, "子代理模型"),
				react.createElement("p", { style: { margin: 0, fontSize: 13, color: "var(--dsw-alias-label-tertiary)" } }, "全局派发规则：所有子代理派发（/side、面板直聊、模型工具发起的一步派发）都会经过它。以「名称 问题」显式指定子代理；未指定时按描述自动匹配最合适的子代理，并严格使用其绑定的模型。"),
				error !== null ? react.createElement("p", { style: { margin: 0, fontSize: 12, color: "var(--dsw-alias-state-error-primary)" } }, error) : null,
				Object.keys(config).length === 0 ? react.createElement("p", { style: { margin: 0, fontSize: 13, color: "var(--dsw-alias-label-tertiary)" } }, "暂无子代理配置。") : null,
				Object.entries(config).map(([entryName, entry]) => react.createElement("div", { key: entryName, style: { display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", border: "1px solid var(--dsw-alias-border-l2)", borderRadius: 8 } },
					react.createElement("span", { style: { fontWeight: 600, fontSize: 13 } }, entryName),
					react.createElement("span", { style: { flex: 1, minWidth: 0, fontSize: 12, color: "var(--dsw-alias-label-tertiary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, (typeof entry.description === "string" && entry.description !== "" ? entry.description + " · " : "") + entry.provider + " / " + entry.model),
					react.createElement("button", { type: "button", style: panelStyles.button, disabled: busy, onClick: () => { void remove(entryName) } }, "删除"),
				)),
				react.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8, padding: 12, borderRadius: 10, background: "var(--dsw-alias-bg-module-platform)" } },
					react.createElement("div", { style: panelStyles.composer },
						react.createElement("input", { style: panelStyles.input, value: name, placeholder: "名称（如 work）", onChange: (event) => { setName(event.target.value) } }),
						react.createElement("input", { style: panelStyles.input, value: desc, placeholder: "描述（用于自动匹配，如：代码与工程任务）", onChange: (event) => { setDesc(event.target.value) } }),
						react.createElement("select", { style: { ...panelStyles.input, maxWidth: 240 }, value: pick, onChange: (event) => { setPick(event.target.value) } },
							react.createElement("option", { value: "" }, "选择模型…"),
							groups.map((group) => react.createElement("optgroup", { key: group.id, label: group.name },
								(group.models ?? []).map((model) => react.createElement("option", { key: group.id + "/" + model.id, value: group.id + "/" + model.id }, model.name)),
							)),
						),
						react.createElement("button", { type: "button", style: panelStyles.button, disabled: busy || name.trim() === "" || pick === "", onClick: () => { void add() } }, "添加"),
					),
				),
			);
		}
