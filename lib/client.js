...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* ---- own side-chat implementation (replaces dsh-sidechain) -----
		   ...
		   The host half registers /side (durable continuable child) and /btw
		   (one-shot background child) on the official subagent fork backend;
		   this client half renders the rest, written from scratch:
		...
		*/
		...
		function SideChatPanel(props) {
			...
			/** Start a brand-new pure-chat side conversation from the panel:
			    the text rides the host /chat command (session.prompt dispatches
			    slash commands), so child creation stays host-side. */
			const startChat = async () => {
				const text = newDraft.trim();
				if (text === "" || api === null || typeof api.sessions?.prompt !== "function") return;
				setBusy(true);
				try {
					...
					if (!result?.result?.ok) throw new Error(result?.result?.message ?? "start failed");
					const commandText = typeof result.result.value?.command?.text === "string" ? result.result.value.command.text : "";
					const childId = CHILD_ID_PATTERN.exec(commandText)?.[0] ?? null;
					setNewDraft("");
			...
			return react.createElement("div", { style: panelStyles.root },
				...
				rows.length === 0 ? react.createElement("p", { style: panelStyles.empty }, "暂无侧边会话——在输入框用 /side 或 /btw 启动一个。") : null,
				react.createElement("div", { style: panelStyles.list },
					rows.map((row) => {
						...
						return react.createElement("button", {
							...
							react.createElement("span", { style: panelStyles.meta }, (row.mode === "continuable" ? "side" : "btw") + (row.activity === "running" ? " · 运行中" : "")),
						);
					}),
				),
				selectedRow !== null ? react.createElement(react.Fragment, null,
					...
					selectedRow.mode === "continuable" ? react.createElement("div", { style: panelStyles.composer },
						react.createElement("input", {
							style: panelStyles.input,
							value: draft,
							placeholder: "回复这个侧边会话…",
							onChange: (event) => { setDraft(event.target.value) },
							onKeyDown: (event) => { if (event.key === "Enter") void sendReply() },
						}),
						react.createElement("button", { type: "button", style: panelStyles.button, disabled: busy || draft.trim() === "", onClick: () => { void sendReply() } }, "发送"),
					) : null,
				...
				// 底部的「直接对话」输入框：始终钉在面板最下面。
				react.createElement("div", { style: panelStyles.composer },
					react.createElement("input", {
						style: panelStyles.input,
						value: newDraft,
						placeholder: "直接对话，开始一个新侧聊（纯对话，无工具）…",
						onChange: (event) => { setNewDraft(event.target.value) },
						onKeyDown: (event) => { if (event.key === "Enter") void startChat() },
					}),
					react.createElement("button", { type: "button", style: panelStyles.button, disabled: busy || newDraft.trim() === "", onClick: () => { void startChat() } }, "开始"),
				),
			);
		}
		...
		/** Command card for /side and /btw: outcome text plus a sidebar jump.
		...
		function SideCommandCard(props) {
			...
			react.useEffect(() => {
				// A fresh child id preselects it; a successful command without a
				// child (e.g. bare /btw) still opens the panel. Both are
				// remembered per browser tab so re-renders never re-trigger.
				const autoKey = childId !== null
					? childId
					: (outcome?.kind === "success" && text !== "" ? "cmd:" + (node?.name ?? "") + ":" + text : null);
				if (autoKey === null || wasAutoOpened(autoKey)) return;
				markAutoOpened(autoKey);
				if (childId !== null) requestSideChatSelection(childId);
				props.openPanel?.();
			}, []);
			// /btw 不留痕：主对话里完全不渲染这张卡片（自动弹栏仍由上面的
			// useEffect 触发）。命令生命周期事件由官方调度器写入、无法
			// 抑制——但 UI 上不可见，提问文本也不落盘（recordInput false）。
		...
		exports.apply = function apply(ctx) {
			...
			// Command cards for /side, /btw, and /chat.
			for (const key of ["side", "btw", "chat"]) {
				ctx.slots.inject("conversation.chat.commandview", () => ctx.slots.register({
					name: "conversation.chat.commandview",
					key,
				}, (props) => SideCommandCard({ ...props, openPanel, hidden: key === "btw" })));
			}
