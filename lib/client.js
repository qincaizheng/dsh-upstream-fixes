...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		 ...
		 * `openSidechainTab` brings the sidebar up and focuses this tab
		 * (a content-seeded openTab also expands a collapsed panel).
		 * @returns a disposer that restores the panel and closes it when the
		 * tab was the one that opened it.
		 */
		function attachSidechainPanel(container, isVisible, openSidechainTab) {
			const ORIGINAL = {};
			let aside = null;
			...
			const frame = () => {
				...
				if (current !== aside) {
					...
					if (aside !== null) {
						...
						integrated = false;
						if (!visible) {
							// The panel just opened through its own toggle /
							// shortcut / a new child while this tab is not in
							// sight — bring it into the sidebar instead of
							// letting the original floating panel take over.
							pendingOpenAt = Date.now();
							try { openSidechainTab(); } catch { pendingOpenAt = 0 }
						}
					}
				}
				if (aside === null) {
				...
				} else if (pendingOpenAt !== 0 && Date.now() - pendingOpenAt < 1500) {
					// Sidebar sliding open for a fresh toggle: stay hidden so
					// the panel never flashes in its floating position.
					if (integrated) { restore(aside); integrated = false; }
					aside.style.setProperty("visibility", "hidden");
				} else {
					// Sidebar closed (and no pending bring-up): hand the panel
					// back to its floating form.
					pendingOpenAt = 0;
					if (integrated) { restore(aside); integrated = false; }
		...
		function SidechainTab(props, service) {
			...
			react.useEffect(() => {
				...
				return attachSidechainPanel(holder.current, () => visibleRef.current, openSidechainTab);
			}, []);
		...
		exports.apply = function apply(ctx) {
			...
			ctx.effect(() => service.registerTab({
				...
				order: 90,
				single: true,
				component: (props) => SidechainTab(props, service),
