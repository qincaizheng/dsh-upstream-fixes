...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function takeRequestedSelection() {
			const id = requestedChildId;
		...
		function SideChatPanel(props) {
			...
			const refresh = react.useCallback(async (options = {}) => {
				...
				try {
					...
					setRows(entries);
					const wanted = takeRequestedSelection();
					if (wanted !== null) {
		...
		exports.apply = function apply(ctx) {
			...
			// Command cards for /side and /btw.
			for (const key of ["side", "btw"]) {
