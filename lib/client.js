...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** Cross-surface draft: bare /btw <question> prefills the panel composer. */
		let requestedDraftText = null;

		function requestSideChatDraft(text) {
			requestedDraftText = text;
		}

		function takeRequestedDraft() {
			const text = requestedDraftText;
			requestedDraftText = null;
			return text;
		}
		...
		function SideChatPanel(props) {
			...
			const refresh = react.useCallback(async (options = {}) => {
				...
				try {
					...
					const wantedDraft = takeRequestedDraft();
					if (wantedDraft !== null) setNewDraft(wantedDraft);
		...
		function SideCommandCard(props) {
			...
			// /btw <问题>：把问题预填进侧栏输入框，而不是自己发子代理。
			const DRAFT_PREFIX = "已打开侧栏面板：";
			const draftFromOutcome = childId === null && outcome?.kind === "success" && text.startsWith(DRAFT_PREFIX) && text.length > DRAFT_PREFIX.length
				? text.slice(DRAFT_PREFIX.length)
				: null;
			const [jumped, setJumped] = react.useState(false);
			react.useEffect(() => {
				// A fresh child id preselects it; a successful command without a
				// child (bare /btw) still opens the panel, and a draft question
				// prefills the composer. All remembered per browser tab so
				// re-renders never re-trigger.
				...
				if (childId !== null) requestSideChatSelection(childId);
				if (draftFromOutcome !== null) requestSideChatDraft(draftFromOutcome);
				props.openPanel?.();
			}, []);
