...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** Class hiding the panel while the sidebar slides open for a fresh
		    toggle (class-based: inline styles stay untouched so the dock
		    capture/restore never picks it up). */
		const BRINGUP_CLASS = "ufx-sidechain-bringup";
		/** Live view of whether the Sidechain tab is the visible one. */
		let sidechainTabVisible = false;
		/** Until when the global bring-up is allowed to hold the panel hidden. */
		let bringUpUntil = 0;
		...
		/**
		 * Dock sidechain's panel into `container` while `isVisible()`. The
		 * bring-up on a fresh toggle is owned by the global watcher (apply);
		 * this attachment only docks, hides, and restores.
		...
		function attachSidechainPanel(container, isVisible) {
			...
			const frame = () => {
				...
				if (visible && onScreen) {
					...
					integrated = true;
					bringUpUntil = 0;
					aside.classList.remove(BRINGUP_CLASS);
				} else if (onScreen) {
		...
		function SidechainTab(props) {
			...
			sidechainTabVisible = props.visible === true;
			react.useEffect(() => {
		...
		/**
		 * Global watcher (always on, even before the tab is ever opened): a
		 * sidechain panel opening while the Sidechain tab is not in sight
		 * brings the sidebar up and focuses the tab — a content-seeded
		 * openTab expands a collapsed panel (type-only opens never do). The
		 * panel is held hidden via BRINGUP_CLASS while the sidebar slides in
		 * so the floating form never flashes; the class drops on dock or
		 * after the bring-up window.
		 */
		function watchSidechainToggle(service) {
			let prev = null;
			let rafId = 0;
			const frame = () => {
				rafId = requestAnimationFrame(frame);
				const aside = findSidechainPanel();
				if (aside !== null && aside !== prev) {
					if (sidechainTabVisible) {
						bringUpUntil = 0;
						aside.classList.remove(BRINGUP_CLASS);
					} else {
						bringUpUntil = Date.now() + 1500;
						aside.classList.add(BRINGUP_CLASS);
						try {
							service.openTab({ type: "sidechain", path: "sidechain" });
						} catch {
							bringUpUntil = 0;
							aside.classList.remove(BRINGUP_CLASS);
						}
				...
				if (aside !== null && (sidechainTabVisible || (bringUpUntil !== 0 && Date.now() > bringUpUntil))) {
					bringUpUntil = 0;
					aside.classList.remove(BRINGUP_CLASS);
				}
				prev = aside;
			...
			return () => {
				cancelAnimationFrame(rafId);
				const aside = findSidechainPanel();
				if (aside !== null) aside.classList.remove(BRINGUP_CLASS);
			};
		}
		...
		exports.apply = function apply(ctx) {
			...
			ctx.effect(() => watchSidechainToggle(service), "upstream-fixes: sidechain toggle interception");
		};
